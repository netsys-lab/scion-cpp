...
/// \brief Abstract interface to P4 dataplane.
class Dataplane
{
public:
    ...
    /// be tried again.
    virtual std::error_code initialize(const std::string& sysfile) = 0;

    /// \brief Release resources and close the sysfile.
    virtual void close() = 0;

    virtual std::error_code printAllCounters(P4Program prog) = 0;
    ...
    /// 64-bit chunks.
    virtual std::error_code tableInsert(
        P4Program prog, const char* name,
        std::span<std::uint64_t> keys,
        const char* action,
        std::span<std::uint64_t> params) = 0;
    ...
    /// depend on the action. Parameters larger than uint64_t must be split into
    /// 64-bit chunks.
    virtual std::error_code tableUpdate(
        P4Program prog, const char* name,
        std::span<std::uint64_t> keys,
        const char* action,
        std::span<std::uint64_t> params) = 0;
    ...
    /// target table. Keys larger than uint64_t must be split into 64-bit
    /// chunks.
    virtual std::error_code tableDelete(
        P4Program prog, const char* name,
        std::span<std::uint64_t> keys) = 0;

    /// \brief Reset counter to zero.
    virtual std::error_code counterReset(P4Program prog, const char* name) = 0;

    /// \brief Read a simple counter.
    virtual scion::Maybe<std::uint64_t> counterSimpleRead(
        P4Program prog, const char* name, std::uint32_t index) = 0;

    /// \brief Write a simple counter.
    virtual std::error_code counterSimpleWrite(
        P4Program prog, const char* name, std::uint32_t index, std::uint64_t value) = 0;

    /// \brief Read a combined packets and bytes counter.
    virtual scion::Maybe<std::pair<std::uint64_t, std::uint64_t>> counterComboRead(
        P4Program prog, const char* name, std::uint32_t index) = 0;

    /// \brief Write a combined packets and bytes counter.
    virtual std::error_code counterComboWrite(
        P4Program prog, const char* name, std::uint32_t index,
        std::uint64_t pkts, std::uint64_t bytes) = 0;
};
